...
This repository contains deployment scripts, systemd service configuration, and CI/CD workflows for [Moltbot](https://molt.bot). Security issues in scope include:

- **Deployment scripts** (`deploy/install.sh`, `deploy/update.sh`, `deploy/setup-server.sh`, `deploy/uninstall.sh`) -- privilege escalation, command injection, insecure file permissions
- **Systemd service configuration** (`deploy/moltbot-gateway.service`) -- sandbox escapes, insufficient hardening
...
- **Environment template** (`deploy/moltbot.env.template`) -- accidental credential exposure

For security issues in the **core Moltbot application**, please report them to the [moltbot/moltbot](https://github.com/moltbot/moltbot) repository instead.
...
- The installer creates a dedicated, non-root `moltbot` system user with limited shell access
...
- `ReadWritePaths` limited to moltbot config, data, and npm directories only
...
When deploying Moltbot, follow these recommendations:

- [ ] **Never run as root.** The installer creates a dedicated `moltbot` user -- always run the service under this account.
- [ ] **Use pairing mode.** Keep `DM_POLICY=pairing` (default) to require approval before the bot responds to new contacts.
- [ ] **Use Tailscale for remote access.** Avoid exposing port 18789 directly to the internet. Use [Tailscale Serve/Funnel](https://docs.molt.bot/gateway/tailscale) instead.
- [ ] **Restrict firewall rules.** Only open ports that are strictly necessary.
- [ ] **Isolate the host.** Run Moltbot on a dedicated VM or VPS that does not contain sensitive data or credentials for other services.
- [ ] **Review skills before installing.** Only install skills from sources you trust.
- [ ] **Keep the system updated.** Regularly apply OS security patches and update Moltbot to the latest version.
